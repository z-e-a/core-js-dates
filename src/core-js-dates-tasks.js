...
function getNextFriday(date) {
  return new Date(
    new Date(date).setDate(date.getDate() + ((7 + 4 - date.getDay()) % 7) + 1)
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    1 + (Date.parse(dateEnd) - Date.parse(dateStart)) / (24 * 60 * 60 * 1000)
  );
}
...
function isDateInPeriod(date, period) {
  return (
    Date.parse(period.start) <= Date.parse(date) &&
...
function getWeekNumberByDate(date) {
  ...
  const daysToNextMonday =
    firstDayOfYear.getDay() === 1 ? 0 : (8 - firstDayOfYear.getDay()) % 7;
  ...
  const weekNumber = Math.ceil(
    (date - nextMonday + 1) / (7 * 24 * 60 * 60 * 1000)
  );
...
function getNextFridayThe13th(date) {
  return Array.from(
    { length: 12 - date.getMonth() },
    (_, i) => date.getMonth() + i
  )
    .map((month) => new Date(date.getFullYear(), month, 13))
    .filter((_13th) => date <= _13th && _13th.getDay() === 5)[0];
}
/**
